...
- Operator census: explorer `GET https://explorer.arcium.com/api/v1/nodes?network=mainnet&limit=100`; keep only `clusterMembership === "active"`; `/api/operators` returns `owner` / `primary` / `boundNode` only (no names, no `src/` imports). Display names from committed `src/data/operator-names.json` on the client (`boundNode` then owner, else truncated primary). Portal scrape is `npm run scrape:operator-names`. `/api` must not import `@arcium-hq/staking`, `@solana/web3.js`, or `@arcium-hq/reader`.
